...
# a <- v0*wgt/price_date[nrow(price_date),c(2,3)]
a <- v0*wgt/prices[1,]
s0 <- prices[1,]
mu <- port_par2$mu_gbm
...
# Parametric VaR1: portfolio follows GBM
GVaR <- gbmVaR(v0,port_par1$mu_gbm,port_par1$sigma_gbm,VaRp,dRtn)
GES <- gbmES(v0,port_par1$mu_gbm,port_par1$sigma_gbm,ESp,dRtn)
plotGraph(GVaR,XOM$date)
plotGraph(GES,XOM$date)
...
PVaR <- parametricVaR(a,s0,mu,sigma,rho,VaRp,dRtn,v0)
plotGraph(PVaR,INTC$date)
